...
export default function useAlan() {
  ...
  const openCart = useCallback(() => {
    isCartEmpty
      ? alanInstance.playText("There is no items in your cart")
  ...
  const closeCart = useCallback(() => {
    isCartEmpty
      ? alanInstance.playText("There is no items in your cart")
      : alanInstance.playText("Closing the cart");
    ...
    ({ detail: { name, quantity } }) => {
      ...
      if (item == null) {
        alanInstance.playText(
          `I could not find the ${name} item, do you want to try again`
        );
      } else {
        addToCart(item.id, quantity);
        alanInstance.playText(`Add ${quantity} of the ${name} to the cart`);
      }
    },
